...
def verif_lt(dut):
    ...
    for i in range (0, nb_sample):
        data = i-(nb_sample/2)
        res_i = int((data*nco_val_i)/pow(2,shift))
        res_q = int((data*nco_val_q)/pow(2,shift))
        #print("lt " + str(i) + " " + str(nb_sample))
        yield send_data(dut, dut.data_lt_i_o, dut.data_lt_q_o, dut.data_lt_en_o,
            data, nco_val_i, nco_val_q, res_i, res_q, 3)
...
def verif_eq(dut):
    ...
    for i in range (0, nb_sample):
        data = i-(nb_sample/2)
        res_i = int((data*nco_val_i))
        res_q = int((data*nco_val_q))
        #print("eq " + str(i) + " " + str(nb_sample))
        yield send_data(dut, dut.data_eq_i_o, dut.data_eq_q_o, dut.data_eq_en_o,
            data, nco_val_i, nco_val_q, res_i, res_q, 2)
